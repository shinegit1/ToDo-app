# write test here for all view classes
import pytest
...
def test_signup_view(client):
    ...
    response2 = client.post(sign_up_url, data1)
    expected_url = "/todoboard/"
    # assert that a user data saved in the database if user redirect to todoboard page
    assertRedirects(response2, expected_url=expected_url, status_code=302)
    ...
    with pytest.raises(Exception):
        response3 = client.post(sign_up_url, data2)
...
@pytest.mark.django_db
@pytest.mark.integration
def test_create_todo_task_view(client):
    user = CustomUser.objects.get(email="dannysmith1@gmail.com")
    todotask_url = f"/create-task/"
    ...
    response = client.post(todotask_url, new_task)
    # assert that the authenticated user created a new todotask if the user redirect to taskboard page
...
def test_todoboard_view(client):
    ...
    response = client.get(todoboard_url, {"object_list": tasks})
    assert response.status_code == 200


@pytest.mark.django_db
@pytest.mark.integration
def test_update_todo_task_view(client):
    user = CustomUser.objects.get(email="dannysmith1@gmail.com")
...
@pytest.mark.django_db
@pytest.mark.integration
def test_delete_todo_task_view(client):
    user = CustomUser.objects.get(email="dannysmith1@gmail.com")
    # get a task of an authenticated user
    task = TodoTask.objects.get(Q(id=1), Q(user=user))
